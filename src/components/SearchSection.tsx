...
export const SearchSection = ({
  ...
  const debouncedShowSuggestions = useMemo(() => debounce(() => {
    setShowSuggestions((filters.q?.length || 0) >= 2 && searchSuggestions.length > 0);
  }, 300), [filters.q, searchSuggestions.length]);
  ...
  useEffect(() => {
    if (isHalalMode && filters.cashAvailable) {
      halalStore.updateState({
        cashAvailable: filters.cashAvailable
      });
    }
  ...
  const handleSearch = async (queryOverride?: string) => {
    ...
    const suggestion = count > 0 ? `Найдено ${count} объектов недвижимости` : 'Попробуйте изменить параметры поиска';
  ...
  const handleSaveSearch = () => {
    ...
    toast({
      ...
      description: 'Добавлен в избранные запросы'
    });
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          ...
          <div className="flex flex-col items-center space-y-3 sm:space-y-0 sm:flex-row sm:items-center sm:space-x-4 shrink-0">
            ...
            <div className={`apple-glow-container flex items-center space-x-3 rounded-full px-4 py-2 ${isHalalMode ? 'apple-glow-active orange-shimmer' : 'apple-glow-inactive cyan-shimmer'}`}>
        ...
        {isHalalMode && <div className="sm:hidden h-4" />}
        ...
        <div>
            ...
            <CardContent className="p-6">
              ...
              <div className="flex gap-3 mb-4">
                  <div className="relative flex-1">
                    ...
                    <Input placeholder={t('search.placeholder')} value={filters.q || ''} onChange={e => handleFilterChange('q', e.target.value)} onFocus={() => setShowSearchHistory(true)} onKeyDown={e => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      handleSearch();
                    }
                    ...
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 flex items-center gap-2">
                      {filters.q && <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={handleSaveSearch}>
                          <BookmarkPlus className="h-3 w-3" />
                        </Button>}
                    </div>

                  {/* Search Suggestions */}
                  {showSuggestions && searchSuggestions.length > 0 && <div className="absolute top-full left-0 right-0 mt-1 bg-background border rounded-md shadow-lg z-50">
                      {searchSuggestions.map(suggestion => <button key={suggestion.id} onClick={() => handleSuggestionClick(suggestion.query)} className="w-full px-3 py-2 text-left hover:bg-muted text-sm flex items-center gap-2">
                          <Clock className="h-3 w-3 text-muted-foreground" />
                          {suggestion.query}
                          {suggestion.resultCount && <Badge variant="outline" className="ml-auto text-xs">
                  ...
                  {/* Search History */}
                  {showSearchHistory && !showSuggestions && (filters.q?.length || 0) < 2 && <div className="absolute top-full left-0 right-0 mt-1 bg-background border rounded-md shadow-lg z-50">
                      <div className="px-3 py-2 border-b">
                        <div className="flex items-center justify-between">
                          <span className="text-sm font-medium">Недавние поиски</span>
                          <Button variant="ghost" size="sm" className="h-6 w-6 p-0" onClick={() => setShowSearchHistory(false)}>
                            <X className="h-3 w-3" />
                          </Button>
                        </div>
                      </div>
                      {getRecentSearches(5).map(item => <button key={item.id} onClick={() => handleSuggestionClick(item.query)} className="w-full px-3 py-2 text-left hover:bg-muted text-sm flex items-center gap-2">
                          <Clock className="h-3 w-3 text-muted-foreground" />
                          {item.query}
                          {item.isHalalMode && <Badge variant="trust" className="text-xs">Халяль</Badge>}
                          {item.resultCount && <Badge variant="outline" className="ml-auto text-xs">
                              {item.resultCount}
                            </Badge>}
                        </button>)}
                      {savedSearches.length > 0 && <>
                          ...
                          {savedSearches.slice(0, 3).map(item => <button key={item.id} onClick={() => handleSuggestionClick(item.query)} className="w-full px-3 py-2 text-left hover:bg-muted text-sm flex items-center gap-2">
                ...
                <Button size="lg" className={`px-8 shadow-warm ${isHalalMode ? 'bg-magit-trust hover:bg-magit-trust/90 text-white' : 'bg-primary hover:bg-primary/90 text-primary-foreground'} transition-all duration-300`} onClick={() => handleSearch()} disabled={searchLoading}>
                  {searchLoading ? <>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={filters.propertyType || 'all'} onValueChange={value => handleFilterChange('propertyType', value === 'all' ? '' : value)}>
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={filters.bedrooms || 'all'} onValueChange={value => handleFilterChange('bedrooms', value === 'all' ? '' : value)}>
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    <Label className="text-sm font-medium flex items-center gap-2">
                     ...
                     <div className="flex gap-2">
                       <Input placeholder={t('filter.min')} value={filters.priceMin || ''} onChange={e => handleFilterChange('priceMin', e.target.value)} className="flex-1" />
                       <Input placeholder={t('filter.max')} value={filters.priceMax || ''} onChange={e => handleFilterChange('priceMax', e.target.value)} className="flex-1" />
                     </div>
                  </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <Input placeholder="35,000" value={filters.cashAvailable || ''} onChange={e => handleFilterChange('cashAvailable', e.target.value)} className="pl-9" />
                        </div>
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <Select value={filters.periodMonths || '12'} onValueChange={value => handleFilterChange('periodMonths', value)}>
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
          ...
          {/* Search Results */}
          {results.length > 0 && <Card className="bg-background/80 backdrop-blur-sm border-0 shadow-warm">
              <CardContent className="p-4 md:p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4 md:gap-6">
                    {/* Show exactly 3 rows for each breakpoint */}
                    <div className="contents sm:hidden">
                      {/* Mobile: 1 column × 3 rows = 3 properties */}
                    ...
                    <div className="contents hidden sm:block lg:hidden">
                      {/* Small: 2 columns × 3 rows = 6 properties */}
                      {results.slice(0, 6).map(property => <PropertyCard key={property.id} id={property.id} title={property.title} location={property.location} price={property.priceUsd} priceUsd={property.priceUsd} bedrooms={property.bedrooms} bathrooms={property.bathrooms} area={property.area} imageUrl={property.image_url} isVerified={property.verified} isHalalFinanced={property.financingAvailable} isHalalMode={isHalalMode} cashAvailable={filters.cashAvailable ? parseFloat(filters.cashAvailable.replace(/[^0-9.]/g, '')) : undefined} financingPeriod={filters.periodMonths ? parseInt(filters.periodMonths) : undefined} onClick={() => handlePropertyClick(property)} />)}
                    </div>
                    <div className="contents hidden lg:block xl:hidden">
                      {/* Large: 3 columns × 3 rows = 9 properties */}
                      {results.slice(0, 9).map(property => <PropertyCard key={property.id} id={property.id} title={property.title} location={property.location} price={property.priceUsd} priceUsd={property.priceUsd} bedrooms={property.bedrooms} bathrooms={property.bathrooms} area={property.area} imageUrl={property.image_url} isVerified={property.verified} isHalalFinanced={property.financingAvailable} isHalalMode={isHalalMode} cashAvailable={filters.cashAvailable ? parseFloat(filters.cashAvailable.replace(/[^0-9.]/g, '')) : undefined} financingPeriod={filters.periodMonths ? parseInt(filters.periodMonths) : undefined} onClick={() => handlePropertyClick(property)} />)}
                    </div>
                    <div className="contents hidden xl:block 2xl:hidden">
                      {/* XL: 4 columns × 3 rows = 12 properties */}
                      {results.slice(0, 12).map(property => <PropertyCard key={property.id} id={property.id} title={property.title} location={property.location} price={property.priceUsd} priceUsd={property.priceUsd} bedrooms={property.bedrooms} bathrooms={property.bathrooms} area={property.area} imageUrl={property.image_url} isVerified={property.verified} isHalalFinanced={property.financingAvailable} isHalalMode={isHalalMode} cashAvailable={filters.cashAvailable ? parseFloat(filters.cashAvailable.replace(/[^0-9.]/g, '')) : undefined} financingPeriod={filters.periodMonths ? parseInt(filters.periodMonths) : undefined} onClick={() => handlePropertyClick(property)} />)}
                    </div>
                    <div className="contents hidden 2xl:block">
                      {/* 2XL: 5 columns × 3 rows = 15 properties */}
                      {results.slice(0, 15).map(property => <PropertyCard key={property.id} id={property.id} title={property.title} location={property.location} price={property.priceUsd} priceUsd={property.priceUsd} bedrooms={property.bedrooms} bathrooms={property.bathrooms} area={property.area} imageUrl={property.image_url} isVerified={property.verified} isHalalFinanced={property.financingAvailable} isHalalMode={isHalalMode} cashAvailable={filters.cashAvailable ? parseFloat(filters.cashAvailable.replace(/[^0-9.]/g, '')) : undefined} financingPeriod={filters.periodMonths ? parseInt(filters.periodMonths) : undefined} onClick={() => handlePropertyClick(property)} />)}
                    </div>
                  </div>
