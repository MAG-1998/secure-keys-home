...
export const Footer = ({
  ...
  return <footer className="border-t border-border/50 bg-background/80 py-12">
      <div className="container mx-auto px-4">
        ...
        <div className="border-t border-border/50 pt-8 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            <Switch checked={theme === "dark"} onCheckedChange={handleThemeToggle} className={isHalalMode ? "data-[state=checked]:bg-magit-trust data-[state=checked]:border-magit-trust [&>span]:data-[state=unchecked]:bg-magit-trust" : "data-[state=checked]:bg-primary data-[state=checked]:border-primary"} />
            <Moon className="h-4 w-4 text-muted-foreground" />
            <div className="flex items-center space-x-2">
              <Label className="text-sm text-muted-foreground">
                {isAutoMode ? <span className="flex items-center gap-1">
                    <Clock className="h-3 w-3" />
                    Auto {theme === "dark" ? "Dark" : "Light"}
                  </span> : "Manual"}
              </Label>
              {!isAutoMode && <button onClick={resetToAutoMode} className="text-xs text-primary hover:text-primary/80 underline">
                  Reset to Auto
                </button>}
            </div>
          </div>
