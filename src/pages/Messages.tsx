  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "messages" }, (payload) => {
        ...
        if (!user) return;
        // If message belongs to any of user's conversations, refetch list
        if (msg.sender_id === user.id || msg.recipient_id === user.id) {
          fetchConversations();
          // If it's for the selected conversation, append
          const otherId = selectedUserId;
          if (otherId && ((msg.sender_id === user.id && msg.recipient_id === otherId) || (msg.sender_id === otherId && msg.recipient_id === user.id))) {
          ...
          {conversations.map((c) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <div className="font-medium">{c.profile?.full_name || c.profile?.email || c.userId}</div>
                  {c.lastMessage && (
      ...
      <Card className="md:col-span-2">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>
            {conversations.find((c) => c.userId === selectedUserId)?.profile?.full_name ||
             ...
             (selectedUserId ? `Chat with ${selectedUserId}` : 'Messages')}
          </CardTitle>
          {selectedUserId && (
        ...
        <CardContent className="flex flex-col h-[70vh]">
          <div className="flex-1 overflow-auto space-y-3 pr-2">
            {messages.map((m) => (
              <div key={m.id} className={`max-w-[80%] p-3 rounded-lg border ${m.sender_id === user.id ? 'ml-auto bg-primary/10' : 'mr-auto bg-muted'}`}>
