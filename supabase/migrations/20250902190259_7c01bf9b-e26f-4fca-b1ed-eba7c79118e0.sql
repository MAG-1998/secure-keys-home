...
BEGIN
  ...
  -- Insert penalty record
  INSERT INTO public.visit_penalties (
    user_id, penalty_type, penalty_level, visit_id, expires_at, moderator_id,
    notes = 'Applied by moderator for no-show'
  ) VALUES (
    user_id_param, 'no_show', penalty_level, visit_id_param, expires_at_date, moderator_id_param
  );
...
-- Cron job to deactivate expired penalties
CREATE OR REPLACE FUNCTION public.deactivate_expired_penalties()
RETURNS INTEGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
