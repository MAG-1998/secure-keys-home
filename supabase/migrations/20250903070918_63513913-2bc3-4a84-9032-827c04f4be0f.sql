-- Function to automatically update financing stage when all documents are submitted
...
BEGIN
  ...
  SELECT stage, user_id, responsible_person_id
  INTO current_stage, request_user_id, responsible_person_id
  ...
  IF pending_docs_count = 0 AND total_docs_count > 0 THEN
    ...
    INSERT INTO halal_financing_activity_log (
      ...
      request_user_id, -- User triggered this by submitting documents
      'stage_change',
      jsonb_build_object(
    ...
    IF responsible_person_id IS NOT NULL THEN
...
-- Trigger function to call auto update when document status changes
CREATE OR REPLACE FUNCTION public.trigger_financing_stage_update()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- Create trigger on document requests table
DROP TRIGGER IF EXISTS trigger_auto_financing_stage_update ON halal_finance_doc_requests;
CREATE TRIGGER trigger_auto_financing_stage_update
  AFTER UPDATE ON halal_finance_doc_requests
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_financing_stage_update();